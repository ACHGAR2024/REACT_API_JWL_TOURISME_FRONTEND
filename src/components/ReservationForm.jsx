...
const ReservationForm = ({ isEditing }) => {
  ...
  useEffect(() => {
    if (isEditing && id) {
      const fetchReservation = async () => {
        try {
          const response = await axios.get(`http://127.0.0.1:8000/api/places_reservations/${id}`, {
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Notification.success(`Réservation ${isEditing ? 'mise à jour' : 'créée'} avec succès !`);

      setTimeout(() => {
        navigate('/reservations');
      }, 2000);
    } catch (error) {
      setError(`Erreur lors de la ${isEditing ? 'mise à jour' : 'création'} de la réservation`);
      console.error(`Erreur lors de la ${isEditing ? 'mise à jour' : 'création'} de la réservation:`, error.response);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-20 mb-72 w-2/3">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="address_place">
            Adresse
          </label>
          <input
            className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
        ...
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="reservation_start_date">
        ...
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="reservation_end_date">
        ...
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="id_events">
            Événement
          </label>
          <select
            className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
