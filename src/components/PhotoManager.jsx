...
const PhotoManager = () => {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/places/${id}/photos`);
  ...
  const handleDelete = async (photoId) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/api/places/${id}/photos/${photoId}`, {
        headers: {
  ...
  return (
    <div className="container mx-auto px-4 py-28">
      <h1 className="text-2xl font-bold mb-4">Gestion des Photos pour la place {id}</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {photos.map(photo => (
          <div key={photo.id} className="relative">
            <img className="w-full h-64 object-cover rounded-lg" src={`http://127.0.0.1:8000${photo.photo_path}`} alt={`Photo ${photo.id}`} />
            <button
