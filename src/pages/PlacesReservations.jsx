...
const PlacesReservations = () => {
  ...
  useEffect(() => {
    ...
    const fetchPlaces = async () => {
      try {
        ...
        const filteredPlaces = allPlaces.filter(place => place.id_events === parseInt(eventId, 10));
        setPlaces(filteredPlaces);
      } catch (error) {
        console.error('Erreur lors de la récupération des places_reservations', error);
      }
    };
  ...
  return (
  <div style={{ height: '100vh', width: '100%' }} className="bg-gray-900 mt-14 mb-24 dark:bg-gray-900 dark:text-white">
  <main className="container mx-auto px-4 py-8">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {places_reservations.map((places_reservation) => (
        <div key={places_reservation.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 animate-fadeIn">
          <div className="p-4">
            <p className="text-gray-500 text-sm mb-2 dark:text-gray-300">{places_reservation.reservation_start_date} - {places_reservation.reservation_end_date}</p>
